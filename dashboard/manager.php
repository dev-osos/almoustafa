    ...
    try {
        ...
        if (!empty($finishedTableExists)) {
                    ...
                    GREATEST(
                        COALESCE(
                            (SELECT MAX(im.created_at) FROM inventory_movements im
                             WHERE im.product_id = fp.product_id AND im.type = 'out'),
                            '2000-01-01'
                        ),
            ...
            foreach ($factoryStagnant as $row) {
                if ($row['last_deduction_at'] === '2000-01-01 00:00:00') {
                    $row['last_deduction_at'] = null;
                }
                $result[] = $row;
            }
        }
        $externalStagnant = $db->query("
            SELECT
                ...
                GREATEST(
                    COALESCE(
                        (SELECT MAX(im.created_at) FROM inventory_movements im
                         WHERE im.product_id = p.id AND im.type = 'out'),
                        '2000-01-01'
                    ),
