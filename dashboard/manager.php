    ...
    try {
            ...
            $factoryStagnant = $db->query("
                SELECT
                    ...
                    (SELECT MAX(im.created_at) FROM inventory_movements im
                     WHERE im.product_id = fp.product_id AND im.type = 'out') AS last_im_out,
        ...
        $externalStagnant = $db->query("
            SELECT
                ...
                NULL AS batch_number, NULL AS production_date,
                p.product_type,
                (SELECT MAX(im.created_at) FROM inventory_movements im
                 WHERE im.product_id = p.id AND im.type = 'out') AS last_im_out,
...
if ($page === 'shipping_orders' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isAjaxShipping && in_array($actionShipping, ['get_shipping_company_statement', 'collect_from_shipping_company', 'deduct_from_shipping_company', 'register_shipping_return', 'get_order_for_return', 'tg_shipments', 'tg_search', 'complete_shipping_order', 'cancel_shipping_order'], true)) {
        while (ob_get_level() > 0) {
            ob_end_clean();
        }
