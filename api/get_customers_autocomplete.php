<?php
/**
 * API للبحث الفوري في العملاء العاديين مع Autocomplete
...
try {
    ...
    // معامل البحث
    $search = trim($_GET['q'] ?? '');
    $section = trim($_GET['section'] ?? ''); // company أو delegates
    ...
    // للمندوب: فقط عملاءه (created_by)
    // للمدير/المحاسب: جميع العملاء أو عملاء المندوبين حسب section
    $sql = "SELECT DISTINCT c.id, c.name, c.phone, c.address, c.balance, 
                   r.name as region_name, u.full_name as created_by_name,
                   c.rep_id, rep.full_name as rep_name
            FROM customers c
    ...
    // تحديد العملاء حسب الدور
    if ($currentRole === 'sales') {
        // المندوب: فقط عملاءه
        $sql .= " AND c.created_by = ?";
        $params[] = $currentUserId;
    } elseif ($section === 'delegates') {
        // عملاء المندوبين فقط
        $sql .= " AND (c.rep_id IS NOT NULL OR c.created_by IN (SELECT id FROM users WHERE role = 'sales'))";
    }
    ...
    $params[] = $searchParam; // c.phone
    $params[] = $searchParam; // c.address
    $params[] = $searchParam; // r.name
    $params[] = $searchParam; // c.id
    ...
    try {
        ...
        foreach ($customers as $customer) {
            ...
            if (!empty($customer['rep_name'])) {
                $subText[] = 'مندوب: ' . htmlspecialchars($customer['rep_name']);
            }
            ...
            $results[] = [
                ...
                'rep_name' => htmlspecialchars($customer['rep_name'] ?? ''),
                'balance' => $balance,
        ...
        error_log('Error in get_customers_autocomplete.php search: ' . $e->getMessage());
        returnJsonResponse([
            'success' => false,
    ...
    error_log('Error in get_customers_autocomplete.php: ' . $e->getMessage());
    returnJsonResponse([
        'success' => false,
