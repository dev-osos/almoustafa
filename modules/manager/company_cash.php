...
<?php
// ── فلتر الشهر الشهري لبطاقة ملخص الخزنة ──────────────────────────────────
$summaryYear  = intval($_GET['summary_year']  ?? date('Y'));
$summaryMonth = intval($_GET['summary_month'] ?? date('n'));
// تحقق من حدود الشهر
if ($summaryMonth < 1)  { $summaryMonth = 12; $summaryYear--; }
if ($summaryMonth > 12) { $summaryMonth = 1;  $summaryYear++; }
$summaryMonthPad  = str_pad($summaryMonth, 2, '0', STR_PAD_LEFT);
$summaryDateStart = "{$summaryYear}-{$summaryMonthPad}-01";
$summaryDateEnd   = date('Y-m-t', mktime(0, 0, 0, $summaryMonth, 1, $summaryYear));
$arabicMonths = ['يناير','فبراير','مارس','أبريل','مايو','يونيو',
                 'يوليو','أغسطس','سبتمبر','أكتوبر','نوفمبر','ديسمبر'];
$summaryMonthLabel = ($arabicMonths[$summaryMonth - 1] ?? '') . ' ' . $summaryYear;
// الشهر السابق / التالي لروابط التنقل
$prevMonth = $summaryMonth - 1; $prevYear = $summaryYear;
if ($prevMonth < 1) { $prevMonth = 12; $prevYear--; }
$nextMonth = $summaryMonth + 1; $nextYear = $summaryYear;
if ($nextMonth > 12) { $nextMonth = 1; $nextYear++; }
// بناء رابط التنقل مع الحفاظ على باقي معاملات الصفحة
function summaryNavUrl(array $base, int $m, int $y): string {
    $p = $base;
    $p['summary_month'] = $m;
    $p['summary_year']  = $y;
    unset($p['ajax']);
...
$navBase = $_GET;
$prevUrl = summaryNavUrl($navBase, $prevMonth, $prevYear);
$nextUrl = summaryNavUrl($navBase, $nextMonth, $nextYear);
$isCurrentMonth = ($summaryYear == date('Y') && $summaryMonth == intval(date('n')));
...
// ── ملخص شهري مفلتر حسب الشهر المختار ──────────────────────────────────────
$monthlySummary = $db->queryOne("
    SELECT
        (SELECT COALESCE(SUM(CASE WHEN type = 'income' AND status = 'approved'
            AND DATE(created_at) BETWEEN ? AND ? THEN amount ELSE 0 END), 0) FROM financial_transactions) +
        (SELECT COALESCE(SUM(CASE WHEN transaction_type IN ('collection_from_sales_rep','income') AND status = 'approved'
            AND DATE(created_at) BETWEEN ? AND ? THEN amount ELSE 0 END), 0) FROM accountant_transactions) AS m_income,

        (SELECT COALESCE(SUM(CASE WHEN type = 'expense' AND status = 'approved'
            AND DATE(created_at) BETWEEN ? AND ? THEN amount ELSE 0 END), 0) FROM financial_transactions) +
        (SELECT COALESCE(SUM(CASE WHEN transaction_type = 'expense' AND status = 'approved'
            AND (description NOT LIKE '%سلفة%' AND description NOT LIKE '%سلف%')
            AND description NOT LIKE '%تسوية رصيد دائن ل%'
        ...
        (SELECT COALESCE(SUM(CASE WHEN type = 'payment' AND status = 'approved'
            AND DATE(created_at) BETWEEN ? AND ? THEN amount ELSE 0 END), 0) FROM financial_transactions) +
        (SELECT COALESCE(SUM(CASE WHEN transaction_type = 'payment' AND status = 'approved'
            AND (description NOT LIKE '%تسوية راتب%' OR description IS NULL)
            AND DATE(created_at) BETWEEN ? AND ? THEN amount ELSE 0 END), 0) FROM accountant_transactions) AS m_payment
",
[
...
$mIncome  = (float)($monthlySummary['m_income']  ?? 0);
$mExpense = (float)($monthlySummary['m_expense'] ?? 0);
$mPayment = (float)($monthlySummary['m_payment'] ?? 0);

// مرتبات الشهر
$mSalaries = 0.0;
if (!empty($salariesTableExists ?? $db->queryOne("SHOW TABLES LIKE 'salaries'"))) {
    $r = $db->queryOne(
        "SELECT COALESCE(SUM(total_amount),0) AS s FROM salaries
         WHERE status IN ('approved','paid') AND DATE(created_at) BETWEEN ? AND ?",
        [$summaryDateStart, $summaryDateEnd]
    );
    $mSalaries = (float)($r['s'] ?? 0);
}

// تسويات المرتبات والسلف في الشهر
$mAdjustments = 0.0;
$mCustomerSettlements = 0.0;
$mManagementSupplies  = 0.0;
if (!empty($accountantTableExists ?? $db->queryOne("SHOW TABLES LIKE 'accountant_transactions'"))) {
    $r2 = $db->queryOne(
        "SELECT COALESCE(SUM(amount),0) AS s FROM accountant_transactions
         WHERE status='approved'
         AND ((transaction_type='payment' AND description LIKE '%تسوية راتب%')
              OR (transaction_type='expense' AND (description LIKE '%سلفة%' OR description LIKE '%سلف%')))
         AND DATE(created_at) BETWEEN ? AND ?",
        [$summaryDateStart, $summaryDateEnd]
    );
    $mAdjustments = (float)($r2['s'] ?? 0);
    ...
    $mCustomerSettlements = (float)($r3['s'] ?? 0);
    ...
    $mManagementSupplies = (float)($r4['s'] ?? 0);
}

$mNet = $mIncome - $mExpense - $mPayment - $mSalaries - $mAdjustments - $mCustomerSettlements - $mManagementSupplies;
$mTotal = $mIncome + $mExpense + $mPayment + $mSalaries + $mAdjustments + $mCustomerSettlements + $mManagementSupplies;
$mDenom = $mTotal > 0 ? $mTotal : 1;
$mIncomeShare     = round(($mIncome / $mDenom) * 100);
$mExpenseShare    = round(($mExpense / $mDenom) * 100);
$mPaymentShare    = round(($mPayment / $mDenom) * 100);
$mSalariesShare   = round(($mSalaries / $mDenom) * 100);
$mAdjShare        = round(($mAdjustments / $mDenom) * 100);
$mCustShare       = round(($mCustomerSettlements / $mDenom) * 100);
$mMgmtShare       = round(($mManagementSupplies / $mDenom) * 100);
    ...
    <div class="col-12 col-xxl-7">
        <div class="card shadow-sm h-100">
            <div class="card-header bg-light fw-bold">
                <!-- السطر الأول: العنوان + زر الطي -->
                <div class="d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-graph-up-arrow me-2 text-primary"></i>ملخص الخزنة الشهري</span>
                ...
                <!-- السطر الثاني: التنقل بين الأشهر -->
                <div class="d-flex align-items-center justify-content-between mt-2">
                    <a href="<?php echo htmlspecialchars($prevUrl, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-sm btn-outline-primary">
                        <i class="bi bi-chevron-right"></i>
                    </a>
                    <span class="fw-semibold text-primary fs-6">
                        <i class="bi bi-calendar3 me-1"></i><?php echo $summaryMonthLabel; ?>
                        <?php if ($isCurrentMonth): ?>
                            <span class="badge bg-success ms-1" style="font-size:0.7rem;">الشهر الحالي</span>
                        <?php endif; ?>
                    </span>
                    <a href="<?php echo htmlspecialchars($nextUrl, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-sm btn-outline-primary <?php echo $isCurrentMonth ? 'disabled' : ''; ?>">
                        <i class="bi bi-chevron-left"></i>
                    </a>
                </div>
            </div>
                ...
                <div class="row g-3 mt-3">
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 h-100">
                            ...
                            <div class="h5 text-success mt-2"><?php echo formatCurrency($mIncome); ?></div>
                            <div class="progress mt-3" style="height: 6px;">
                                <div class="progress-bar bg-success" role="progressbar" style="width: <?php echo max(0, min(100, $mIncomeShare)); ?>%;"></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?php echo max(0, min(100, $mIncomeShare)); ?>% من إجمالي الحركة</small>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 h-100">
                            ...
                            <div class="h5 text-danger mt-2"><?php echo formatCurrency($mExpense); ?></div>
                            <div class="progress mt-3" style="height: 6px;">
                                <div class="progress-bar bg-danger" role="progressbar" style="width: <?php echo max(0, min(100, $mExpenseShare)); ?>%;"></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?php echo max(0, min(100, $mExpenseShare)); ?>% من إجمالي الحركة</small>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 h-100">
                            ...
                            <div class="h5 text-warning mt-2"><?php echo formatCurrency($mPayment); ?></div>
                            <div class="progress mt-3" style="height: 6px;">
                                <div class="progress-bar bg-warning" role="progressbar" style="width: <?php echo max(0, min(100, $mPaymentShare)); ?>%;"></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?php echo max(0, min(100, $mPaymentShare)); ?>% من إجمالي الحركة</small>
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 h-100">
                            ...
                            <div class="h5 text-info mt-2"><?php echo formatCurrency($mAdjustments); ?></div>
                            <div class="progress mt-3" style="height: 6px;">
                                <div class="progress-bar bg-info" role="progressbar" style="width: <?php echo max(0, min(100, $mAdjShare)); ?>%;"></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?php echo max(0, min(100, $mAdjShare)); ?>% من إجمالي الحركة</small>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 h-100">
                            ...
                            <div class="h5 text-secondary mt-2"><?php echo formatCurrency($mCustomerSettlements); ?></div>
                            <div class="progress mt-3" style="height: 6px;">
                                <div class="progress-bar bg-secondary" role="progressbar" style="width: <?php echo max(0, min(100, $mCustShare)); ?>%;"></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?php echo max(0, min(100, $mCustShare)); ?>% من إجمالي الحركة</small>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 h-100">
                            ...
                            <div class="h5 text-dark mt-2"><?php echo formatCurrency($mManagementSupplies); ?></div>
                            <div class="progress mt-3" style="height: 6px;">
                                <div class="progress-bar bg-dark" role="progressbar" style="width: <?php echo max(0, min(100, $mMgmtShare)); ?>%;"></div>
                            </div>
                            <small class="text-muted d-block mt-2"><?php echo max(0, min(100, $mMgmtShare)); ?>% من إجمالي الحركة</small>
                        </div>
                    </div>
                </div>
